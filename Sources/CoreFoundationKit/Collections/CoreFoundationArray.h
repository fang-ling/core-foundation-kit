 ...
 *     The values in the new array are ordered in the same order in which they appear in this C array.
 *     This value may be `null` if `count` is 0.
 *     This C array is not changed or freed by this function.
 *     If values is not a valid pointer to a C array of at least `count` elements, the behavior is undefined.
 *   - count: The number of values to copy from the values C array into the new array.
 *     This number will be the count of the new array—it must not be negative or greater than the number of elements in values.
 */
CoreFoundationArray* CoreFoundationArrayInitialize(const void* nillable* nillable objects, CInteger count);
 ...
 *     If the index is outside the index space of the array (`0` to `N-1` inclusive (where `N` is the count of the array)), the behavior is undefined.
 *
